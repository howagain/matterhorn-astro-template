...
export default function Coupon(props: Props): JSX.Element {
  ...
  const containerStyle = () =>
    ({
      ...
      "grid-template-columns":
        coupon().layout === "horizontal" ? "1fr 1.2fr" : undefined,
  ...
  const imageStyle = () =>
    ({
      "background-image": coupon().backgroundImage
        ? `url(${coupon().backgroundImage})`
        : undefined,
  ...
  const body = (
    <div style={containerStyle()}>
      ...
      <div>
        <h3 style={{ margin: "0 0 0.4rem", "font-size": "1.25rem" }}>
          {coupon().title}
        </h3>
        <p style={{ margin: "0 0 0.6rem", color: "#334155" }}>
          {coupon().description}
        </p>
        ...
        <Show
          when={
            coupon().customFields &&
            Object.keys(coupon().customFields || {}).length > 0
          }
        >
          <div
            style={{
  ...
  return <Show when={!isModal() || showModal()}>{body}</Show>;
}
